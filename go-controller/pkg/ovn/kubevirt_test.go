...
var _ = Describe("OVN Kubevirt Operations", func() {
	...
	var (
		...
		// mac
		virtLauncher = func(suffix, node, vm, ipFamily string, extraLabels, extraAnnotations map[string]string) testVirtLauncherPod {
	...
	Context("during execution", func() {
		DescribeTable("reconcile migratable vm pods", func(t testData) {
			...
			app.Action = func(ctx *cli.Context) error {
				...
				if t.migrationTarget.nodeName != "" {
					t.migrationTarget.populateLogicalSwitchCache(fakeOvn, getLogicalSwitchUUID(fakeOvn.controller.nbClient, t.migrationTarget.nodeName))
				}
			...
			Entry("for pre-copy live migration to node owning subnet at global zone", testData{
				...
				testVirtLauncherPod: virtLauncher1WithExtraLabels(node2, vm1, "dualstack",
					map[string]string{
