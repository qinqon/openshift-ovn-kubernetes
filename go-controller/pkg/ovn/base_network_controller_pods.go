...
func (bnc *BaseNetworkController) allocatePodIPs(pod *kapi.Pod,
	...
	if bnc.lsManager.IsNonHostSubnetSwitch(switchNames.Original) {
		return "", nil
	}
...
func (bnc *BaseNetworkController) addLogicalPortToNetwork(pod *kapi.Pod, nadName string,
	...
	pod, err = kubevirt.EnsureNetworkInfoForVM(bnc.watchFactory, bnc.kube, pod, bnc.GetNetworkName())
	if err != nil {
...
func (bnc *BaseNetworkController) getSwitchNames(pod *corev1.Pod) (kubevirt.SwitchNames, error) {
	switchNames := kubevirt.SwitchNames{}
	var err error
	...
	if kubevirt.IsPodLiveMigratable(pod) && ok {
